...
function popupCardHTML(p: MapPoint) {
  ...
  <div style="width:280px; border:1px solid #e5e7eb; border-radius:12px; overflow:hidden; background:#fff;">
    <div style="position:relative; aspect-ratio:16/9; background:#f4f4f5;">
      ...
      ${precoNum === -1 ? `
        <span style="
          position:absolute; top:18px; right:-40px; background:#dc2626; color:#fff;
          font-weight:700; font-size:12px; padding:6px 50px; transform:rotate(45deg);
          box-shadow:0 2px 6px rgba(0,0,0,.25); letter-spacing:.05em; text-align:center; display:inline-block; white-space:nowrap;
        ">VENDIDO</span>` : ""}
      ${hasDiscount ? `
        <span style="position:absolute; top:8px; left:8px; background:#e11d48; color:#fff; font-size:12px; padding:2px 8px; border-radius:6px;">
          Preço baixou
        </span>` : ""}
    </div>
    <div style="padding:12px;">
      ...
      ${precoStr ? `
        <div style="font-size:14px; font-weight:600; margin-bottom:8px; color:${precoStr === "VENDIDO" ? "#dc2626" : "#065f46"};">
          ${precoStr}
        </div>` : ""}
...
/* ==== Fix de layout/z-index/overscroll (sem “dobrar” espaçamento) ==== */
function injectLayoutFix() {
  const id = "plink-leaflet-layout-fix";
  if (document.getElementById(id)) return;
  const style = document.createElement("style");
  style.id = id;
  style.textContent = `
    .plink-map { position: relative; z-index: 0; overflow: hidden; overscroll-behavior: contain; }
    .plink-map .leaflet-container { isolation: isolate; }
    .plink-map .leaflet-top, .plink-map .leaflet-bottom { z-index: 120; }
    .plink-map .leaflet-pane, .plink-map .leaflet-control { z-index: 110; }
    .plink-map .leaflet-popup { z-index: 140; }
  `;
  document.head.appendChild(style);
}

/* ==== Util: altura do header para padding do autoPan ==== */
function getHeaderHeight(): number {
  // pegue o seu header sticky. Ajuste o seletor se necessário.
  const h =
    (document.querySelector("header.sticky, header[data-sticky='true']") as HTMLElement) ||
...
export default function MapClient({
  ...
  const [drawState, setDrawState] = useState<{ mode: "idle" | "drawing" | "done"; verts: Array<{ lat: number; lng: number }>; }>({ mode: "idle", verts: [] });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const headerPad = Math.max(0, Math.floor(getHeaderHeight())); // px
        ...
        for (const p of points) {
          ...
          marker.bindPopup(popupCardHTML(p), {
            ...
            // evita “pulo” e considera o header sticky
            autoPan: true,
            autoPanPaddingTopLeft: [0, headerPad + 12],
            autoPanPaddingBottomRight: [0, 12],
            keepInView: true,
            closeButton: true,
          });
        ...
        // impedir que o mapa provoque scroll do documento
        map.on("popupopen", () => {
          try { map.scrollWheelZoom.enable(); } catch {}
        });
        map.on("popupclose", () => {
          // nada de mexer em window.scrollTo aqui
        });
        ...
        function refreshTempLayers(verts: Array<{ lat: number; lng: number }>) {
          if (!tempLineRef.current) {
            tempLineRef.current = L.polyline([], { color: "#2563eb", weight: 2, dashArray: "6 4" }).addTo(map);
          }
          tempLineRef.current.setLatLngs(verts.map((v) => L.latLng(v.lat, v.lng)));

          if (verts.length >= 3) {
            if (!tempPolyRef.current) {
              tempPolyRef.current = L.polygon([], { color: "#2563eb", fillColor: "#3b82f6", fillOpacity: 0.1 }).addTo(map);
            }
          ...
          verts.forEach((v) => {
            L.circleMarker([v.lat, v.lng], { radius: 4, color: "#2563eb", weight: 2, fillColor: "#ffffff", fillOpacity: 1 })
        ...
        const obs = new ResizeObserver(() => { try { map && map.invalidateSize(); } catch {} });
  ...
  return (
    <div ref={outerRef} className={`${className || ""} relative z-0`} style={{ ...(style || {}) }}>
      ...
      {(enableDraw ?? points.length > 1) && (
        ...
        >
          {drawState.mode === "idle" && (
            <button onClick={startDraw} className="px-3 py-2 rounded-lg bg-[#0a454f] text-white text-sm font-semibold hover:opacity-90">
              Desenhar área
            </button>
          )}
          ...
          {drawState.mode === "drawing" && (
            <>
              <button onClick={finishDraw} className="px-3 py-2 rounded-lg bg-[#0a454f] text-white text-sm font-semibold hover:opacity-90">
                Finalizar
              </button>
              <button onClick={clearDraw} className="px-3 py-2 rounded-lg border text-sm">Limpar</button>
              <span className="self-center text-xs text-zinc-700">Clique no mapa para adicionar vértices • Duplo-clique para fechar</span>
            </>
          )}

          {drawState.mode === "done" && (
            <>
              <button onClick={goToResults} className="px-3 py-2 rounded-lg bg-[#0a454f] text-white text-sm font-semibold hover:opacity-90">
                Ver imóveis na área
              </button>
              <button onClick={clearDraw} className="px-3 py-2 rounded-lg border text-sm">Limpar</button>
